...
namespace Untek\Framework\Messenger\Presentation\Cli\Commands;
...
//        ContainerInterface $rateLimiterLocator = null
    )
    {
        ...
        $this->busIds = $busIds;
        $this->rateLimiterLocator = null; // $rateLimiterLocator;
            ...
            ->setDefinition(
                [
                    ...
                    new InputOption(
                        ...
                        'Do not reset container services after each message'
                    ),
                ]
            )
            ->setHelp(
    ...
    protected function interact(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($this->receiverNames && !$input->getArgument('receivers')) {
            ...
            if (\count($this->receiverNames) > 1) {
                $io->writeln(
                    sprintf(
                        'Hint: to consume from multiple, use a list of their names, e.g. <comment>%s</comment>',
                        implode(', ', $this->receiverNames)
                    )
                );
            }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (null !== $limit = $input->getOption('limit')) {
            if (!is_numeric($limit) || 0 >= $limit) {
                throw new InvalidOptionException(
                    sprintf('Option "limit" must be a positive integer, "%s" passed.', $limit)
                );
            }
        ...
        if ($memoryLimit = $input->getOption('memory-limit')) {
            ...
            $this->eventDispatcher->addSubscriber(
                new StopWorkerOnMemoryLimitListener($this->convertToBytes($memoryLimit), $this->logger)
            );
        }
        ...
        $io->success(
            sprintf(
                'Consuming messages from transport%s "%s".',
                \count($receivers) > 1 ? 's' : '',
        ...
        $bus = $input->getOption('bus') ? $this->routableBus->getMessageBus(
            $input->getOption('bus')
        ) : $this->routableBus;

        $worker = new Worker($receivers, $bus, $this->eventDispatcher, new NullLogger(), $rateLimiters);
    ...
    public function complete(CompletionInput $input, CompletionSuggestions $suggestions): void
    {
        if ($input->mustSuggestArgumentValuesFor('receivers')) {
            $suggestions->suggestValues(
                array_diff(
                    $this->receiverNames,
                    array_diff($input->getArgument('receivers'), [$input->getCompletionValue()])
                )
            );
